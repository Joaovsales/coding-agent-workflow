...
| `aposd-design-reviewer` | `sonnet` | APOSD structural design review (read-only) — depth, leakage, temporal decomposition, unknown unknowns |
...
- **Coding agents** (`backend-developer`, `frontend-developer`, `code-reviewer`, `code-debugger`, `frontend-design-validator`, `context-document-optimizer`, `security-reviewer`, `critic`, `aposd-design-reviewer`): `model: "sonnet"`
...
| `/session-design-review` | End-of-session APOSD design **tutorial** for learning — based on 'A Philosophy of Software Design' by John Ousterhout |
| `/aposd-design-review` | APOSD structural design **gate** — runs the `aposd-design-reviewer` agent on changed files, maps findings to MUST-FIX/SHOULD-FIX/NITPICK, and issues a GO/HOLD/STOP verdict. Auto-invoked by `/build` Phase 3.5. |
