...
# /aposd-design-review — APOSD Design Quality Gate
...
**When to run:**
- Manually: `/skill:aposd-design-review` after you want a design-only sanity check
...
**How to run:**
```
/skill:aposd-design-review              # Review all changed files in current branch
/skill:aposd-design-review --file path  # Scope to one file
/skill:aposd-design-review --scope auth # Scope to functional area (grep + diff)
```
...
For each changed file (or grouped batch if <5 files), dispatch the `aposd-design-reviewer` agent (`model: sonnet`) in a single tool call. Pass:
...
- `.claude/skills/session-design-review/references/principles.md` — Full APOSD principles catalog
- `.claude/agents/aposd-design-reviewer.md` — The agent prompt and red flag definitions
